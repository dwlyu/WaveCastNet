...
def getsequenceData(input_array,
    ...
    '''    
    return get_data_loader(input_array,input_steps,future_steps,down_sample,cropx,cropy,spatial_ds,batch_size,pin_memory,shuffle)

def get_data_loader(input_array,input_steps,future_steps,down_sample,cropx,cropy,spatial_ds,batch_size,pin_memory,shuffle): 
    seed = 3938
    np.random.seed(seed)
    ...
    dataset = GetClimateDataset(input_array, transform, input_steps,future_steps,down_sample,cropx,cropy,spatial_ds) 
...
class GetClimateDataset(Dataset):
    ...
    def __init__(self, input_array, transform, input_steps,future_steps,down_sample,cropx,cropy,spatial_ds):
    ...
    def __getitem__(self, global_idx):
        ...
        X = self.transform(self.input[sample_idx,:,self.cropx[0]:self.cropx[1]:self.spatial_ds,self.cropy[0]:self.cropy[1]:self.spatial_ds, local_idx:local_idx + 
                                      self.down_sample*self.num_snapshots : self.down_sample]).permute(0,3,1,2)
        
        y = self.transform(self.input[sample_idx,:,self.cropx[0]:self.cropx[1]:self.spatial_ds,self.cropy[0]:self.cropy[1]:self.spatial_ds, local_idx + self.down_sample*self.num_snapshots:local_idx + 
