...
from src.models_earthquake._init_ import Validation_pixel, getsequenceData, getsequenceData_uq
import gc
...
parser.add_argument("--retrain", type=bool, default=False, help="Uncertainty quantification")
...
# Loading Data
#==============================================================================
inputlist = np.load('WaveCastNet/data/earthquake/inputlist.npy')
testlist = np.load('WaveCastNet/data/earthquake/testlist.npy')[::3]
...
if args.training_uq:
    ...
    for i in range(15):     
        rand = np.random.choice(np.arange(0, 20), size=4, replace=False)
        for r in rand:
            file_id = (int)(r/4)
            idx = (int)(r%4)
    ...
    inputlist = np.load("WaveCastNet/data/earthquake/all_input.npy", mmap_mode='r')
    inputlist = inputlist[indices_to_keep]
    print(inputlist.shape)
    print('finish loading data...............................')

    testlist = np.load("WaveCastNet/data/earthquake/all_validation.npy")
    print(testlist.shape)
    print('finish loading data...............................')
    
    train_loader = getsequenceData_uq(inputlist,input_steps=30, future_steps=30,batch_size=args.batch_size, pin_memory=True)
    val_loader = getsequenceData_uq(testlist,input_steps=30, future_steps=195, batch_size=args.batch_size_test, pin_memory=True)
    step = 30

else:
    if args.model == 'LEM_dense' or args.model == 'LEM_sparse':
        train_loader = getsequenceData(inputlist,sample_per_file=60,input_steps=30, future_steps=30,batch_size=args.batch_size,
                                    down_sample=2, spatial_ds=1,pin_memory=True)
        val_loader = getsequenceData(testlist,sample_per_file=15, input_steps=30, future_steps=195, batch_size=args.batch_size_test,
                                    down_sample=2, spatial_ds=1,pin_memory=True)
        step = 30
    else:
        train_loader = getsequenceData(inputlist,sample_per_file=60, input_steps=60, future_steps=60,batch_size=args.batch_size,
                                    cropx=[24,360], cropy=[16,240], down_sample=1, spatial_ds=args.spatial_ds,pin_memory=True)
        val_loader = getsequenceData(testlist,sample_per_file=15, input_steps=60, future_steps=390, batch_size=args.batch_size_test,
                                    cropx=[24,360], cropy=[16,240], down_sample=1, spatial_ds=args.spatial_ds,pin_memory=True)
        step = 60
...
OUT_DIR = os.path.join(DESTINATION_PATH, f'arch_{args.model}_dim_{args.kernel_size}_lr_{args.learning_rate}_seed_{args.seed}')
    
#==============================================================================
# Retraining Setup
#==============================================================================

# state_dict  = torch.load(f'/pscratch/sd/d/dwlyu/new_data/{model_name}_.pt')
# model.load_state_dict(state_dict=state_dict)
    
# trainloss_list = list(np.load(f'/pscratch/sd/d/dwlyu/new_data/{model_name}_train.npy'))
# validMSE = list(np.load(f'/pscratch/sd/d/dwlyu/new_data/{model_name}_MSE.npy'))
# validRFNE = list(np.load(f'/pscratch/sd/d/dwlyu/new_data/{model_name}_RFNE.npy'))
# validRMSE = list(np.load(f'/pscratch/sd/d/dwlyu/new_data/{model_name}_RMSE.npy'))
# validACC = list(np.load(f'/pscratch/sd/d/dwlyu/new_data/{model_name}_ACC.npy'))
    
# best_error= np.inf
# checkpoint_dir = '/pscratch/sd/d/dwlyu/new_data/'

# checkpoint_path = f'{checkpoint_dir}checkpoint_{model_name}_.pt'
