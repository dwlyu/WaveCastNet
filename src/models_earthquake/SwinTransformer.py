...
class UpSample(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.reshape(x, (B,D,H,W, 2, 2, x.shape[-1]//4))
        # Change the order of x
        x = x.permute(0,1,2,4,3,5,6)
        # Reshape to get Tensor with a resolution of (8, 360, 182)
        x = torch.reshape(x, (x.shape[0], D, 2*H, 2*W, x.shape[-1]))
...
class SwinTransformer3D_eq(nn.Module):
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            ...
            if i == 0:
                ...
                skip = x
                pad_input = (skip.shape[3] % 2 == 1) or (skip.shape[4] % 2 == 1)
                if pad_input:
                    skip = F.pad(skip, (0, skip.shape[4] % 2, 0, skip.shape[3] % 2,0,0))
                
                x = self.merge(rearrange(x, 'b c d h w  -> b d h w c'))
                x = rearrange(x, 'n d h w c -> n c d h w')

            i += 1

        return self.reverse(torch.concat((x,skip),dim=1))[:,:,:origin_d,:origin_h,:origin_w]
